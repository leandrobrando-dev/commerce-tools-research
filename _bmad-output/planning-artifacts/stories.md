...
Epic 1 is split into three named sub-phases — **1A / 1B / 1C** — covering platform scaffold (1A), Migrator MVP (1B), and Migrator Phase 2 (1C). Stories 1.1–1.7 (Phase 1A) appear immediately below; Stories 1.8–1.14 (Phase 1B + 1C) physically live in the document section currently labelled "Epic 4 (Dissolved)" — they were renumbered during the 2026-05-12 epic restructure but were not physically moved (preserves git diff history). See [epics.md → Epic 1](epics.md#epic-1-foundation-migration--operator-workspace) for the full per-phase prose.

**Phase 1A — Platform Scaffold** _(below)_ — Stories 1.1–1.7
**Phase 1B — Migrator MVP** _(in dissolved-Epic-4 section below)_ — Stories 1.8, 1.9, 1.10, 1.13, 1.14
**Phase 1C — Migrator Phase 2** _(deferrable; in dissolved-Epic-4 section below)_ — Stories 1.11, 1.12
...
Epic 3 is split into two named sub-phases — **3A / 3B** — covering the Storefront Editor + Publishing (3A) and the AI Site Builder warm-start completion engine (3B). Stories 3.0–3.14 (Phase 3A) appear immediately below; Stories 3.15–3.20 (Phase 3B) physically live in the document section currently labelled "Epic 4 (Dissolved)" — they were renumbered during the 2026-05-12 epic restructure but were not physically moved (preserves git diff history). See [epics.md → Epic 3](epics.md#epic-3-storefront-editor-publishing--ai-site-builder) for the full per-phase prose.

**Phase 3A — Storefront Editor & Publishing** _(below)_ — Stories 3.0, 3.1a, 3.1b, 3.2–3.14
**Phase 3B — AI Site Builder (Warm-Start Completion)** _(in dissolved-Epic-4 section below)_ — Stories 3.15, 3.16a, 3.16b, 3.16c, 3.17, 3.18, 3.19, 3.20
...
**FRs Covered:** FR51
**Complexity:** M
## ~~Epic 4: AI Site Builder & Migrator~~ — **Dissolved (2026-05-12)** _(stories below physically remain here for git-diff continuity; conceptually they live under Epic 1 and Epic 3)_

Per the 2026-05-12 epic restructure, Epic 4 dissolved. Migration scope (former Stories 4.1, 4.8, 4.9, 4.10, 4.11, 4.12, 4.13) was renumbered to **Stories 1.8–1.14** and now belongs to **Epic 1 Phase 1B/1C**. AI Site Builder scope (former Stories 4.2, 4.3a/b/c, 4.4, 4.5, 4.6, 4.7) was renumbered to **Stories 3.15–3.20** and now belongs to **Epic 3 Phase 3B**. The story bodies below were not physically moved during the rewrite — preserves git diff history for unchanged story content. Read each story under its conceptual epic membership (Epic 1 or Epic 3 per the per-story section).

**Migration stories (Epic 1 Phase 1B + 1C)** in document order below: Story 1.8, Stories 1.9–1.14 (interleaved with the AI Site Builder stories).

**AI Site Builder stories (Epic 3 Phase 3B)** in document order below: Stories 3.15, 3.16a, 3.16b, 3.16c, 3.17, 3.18, 3.19, 3.20.

See [epics.md → Epic 1](epics.md#epic-1-foundation-migration--operator-workspace) and [epics.md → Epic 3](epics.md#epic-3-storefront-editor-publishing--ai-site-builder) for full per-phase prose, ship gates, and FR coverage.

---

### Story 1.8: CT Project Connection via API Credentials  *(Epic 1 Phase 1B)*
...
### Story 3.15: Commerce Intelligence Drawer — Create Mode Infrastructure
...
6. Opening the drawer in Create mode does not trigger any AI inference call on its own; inference is triggered only by observed canvas edit sequences (Stories 3.16a–3.16c); the drawer shows an idle state with instructional copy until inference begins.
...
**Dependencies:** Story 1.8, Inngest 4.2.6 app bootstrap
**FRs Covered:** FR7, FR65
**Complexity:** XL

---

### Story 3.16a: Canvas Edit Observation Hook & Inngest Event Firing
...
**Dependencies:** Story 3.15 (Drawer infrastructure), Story 3.1a (canvas state manager exists), Inngest 4.2.6
**FRs Covered:** FR7 (edit-observation portion)
**Complexity:** M

---

### Story 3.16b: AI Inference Engine — OpenRouter Integration with Library-Governed Prompt
...
**Dependencies:** Story 3.16a (`intent-observed` event source), Story 3.15 (drawer to receive results), published component library CT Custom Object (FR77 default + tenant additions), OpenRouter API key in `securedConfiguration`, Upstash Ratelimit SDK
**FRs Covered:** FR7 (inference portion), FR8 (governed library scoping)
**Complexity:** L

---

### Story 3.16c: Streaming Pipeline, Zod Validation, and Drawer Wiring
...
**Acceptance Criteria:**
1. The `site-builder/ai-completion-job` Inngest function streams partial candidate JSON to the drawer via Inngest's `step.sendEvent` mechanism — the drawer subscribes to `site-builder/candidate-progress` events and renders intermediate state in the `AIReasoningCard` (covered by Story 3.17) as text streams in.
...
3. A 30-second hard timeout is enforced via `Promise.race` between OpenRouter stream resolution and `step.sleep('30s')` abort; on timeout, the function emits a `TIMEOUT` event, the drawer shows a graceful error state with "Try again" and "Cancel" CTAs, and the streaming canvas skeleton (Story 3.20) is removed.
...
5. Drawer cancellation (operator clicks Cancel or closes the drawer) sends a `site-builder/cancel-inference` event consumed by the Inngest function; the function aborts the OpenRouter stream cleanly within 1 s, releases the rate-limit token (per Story 3.16b AC7), and stops emitting progress events.
6. Inference results are never applied to the canvas without operator approval (covered by Story 3.17); the raw candidate is held in React state in the drawer and discarded on drawer close, Cancel action, or successful Apply.
...
**Dependencies:** Story 3.16a, Story 3.16b, Story 3.17 (`AIReasoningCard` is the streaming target — but 4.3c can ship against a stub card and 4.4 enhances it), Inngest 4.2.6 with `step.sendEvent`, Zod
**FRs Covered:** FR7 (streaming + cancellation portion), FR12 (streaming progress feedback wired into the drawer)
**Complexity:** M

---

### Story 3.17: AIReasoningCard / ConfidenceCard UI
...
**Dependencies:** Story 3.16c (drawer subscribes to streaming candidate events from 3.16c; 3.16a + 3.16b are transitive dependencies), Inngest candidate event
**FRs Covered:** FR9
**Complexity:** L

---

### Story 3.18: Governed AI Completion Application
...
**Dependencies:** Story 3.17, canvas state manager, CT Custom Objects API
**FRs Covered:** FR8, FR10, FR11
**Complexity:** L

---

### Story 3.19: Accept/Modify/Reject Individual AI Placements
...
**Dependencies:** Story 3.17, Story 3.18
**FRs Covered:** FR10, FR11
**Complexity:** M

---

### Story 3.20: AI Completion Streaming Progress Feedback
...
**Dependencies:** Story 3.15, Story 3.16a (edit observation provides skeleton overlay trigger), Story 3.16c (streaming wiring carries the progress events)
**FRs Covered:** FR12
**Complexity:** M

---

### Story 1.9: Historical Behavioral Data Import
...
**Dependencies:** Story 1.8, behavioral_events table (Epic 1), Inngest 4.2.6
**FRs Covered:** FR63
**Complexity:** L

---

### Story 1.10: Frontastic Migration Path
...
**Dependencies:** Story 1.8, published component library CT Custom Object, OpenRouter, Story 1.13
**FRs Covered:** FR34, FR35, FR36, FR37
**Complexity:** XL

---

### Story 1.11: Custom Next.js Migration Path
...
4. Extracted components are matched against the published component library using the same two-pass strategy as Story 1.10; each match result includes `inferredZone` (`"green"` if CT SDK usage detected and maps to library, `"red"` otherwise) and a `scaffoldPriority` score.
5. The developer review UI (Story 1.13) receives the extraction results and renders them with file paths, prop diffs between the extracted interface and the library component schema, and the inferred zone; developers can override zone assignment and edit field mappings inline.
...
**Dependencies:** Story 1.8, GitHub App registration, Neon branch database, OpenRouter, Inngest 4.2.6, Story 1.13
**FRs Covered:** FR34, FR35, FR36, FR37
**Complexity:** XL

---

### Story 1.12: Monolith Migration Path + Core Web Vitals Baseline
...
**Acceptance Criteria:**
1. The CWV baseline tool is accessible from a standalone route (`/migrate/cwv-baseline`) that requires only a valid CT project connection (Story 1.8) and does NOT require a full platform subscription; a "Pre-sales mode" banner is shown when accessed without full activation.
...
7. The generated canvas drafts are written as CT Custom Objects (`container: "migration-canvas-drafts"`, `key: "{pageType}-{projectKey}"`) and surfaced in the platform canvas editor as "Migration Drafts" — editable before publish; a readiness check (Story 1.14) is automatically queued for all generated drafts.

**Dependencies:** Story 1.8, Google PageSpeed Insights API key, CT Custom Objects API, OpenRouter, Inngest 4.2.6, Story 1.13, Story 1.14
**FRs Covered:** FR34, FR35, FR36, FR37, FR38, FR39
**Complexity:** XL

---

### Story 1.13: Migration Scaffolded Component Mapping Review
...
8. A test-fixture set is committed to the repository at `tests/fixtures/migration-mapping-{frontastic|nextjs|monolith}.json` covering at minimum: (a) a happy-path mapping where every component matches with `matchConfidence ≥ 0.85`, (b) an unmatched-component scenario producing Red Zone defaults, and (c) a field-type-mismatch scenario surfacing the field-mapping drawer's coercion strategies. The development environment seeds these fixtures into a CT Custom Object stub (`container: "migration-mapping"`, `key: "fixture-{path}"`) on `next dev` startup via an Inngest function `migration/seed-fixtures`; this allows Story 1.13 to be developed, QA'd, and merged before any of Stories 1.10 / 1.11 / 1.12 lands.

**Dependencies:**
- *Build deps:* Test-fixture migration mapping JSON (per AC8); published component library CT Custom Object; Neon branch database.
- *Forward consumers:* Stories 1.10, 1.11, 1.12 hand off their generated mappings to this review UI for publish.
- *Not blocked by:* Stories 1.10, 1.11, 1.12. Story 1.13 is fixture-driven in development per AC8 and ships independently. End-to-end production validation requires at least one migration path (Story 1.10 first — Frontastic, per Priority Sequencing P2) to be wired through after 1.13 lands.
...
### Story 1.14: Migrated Page Validation & Readiness Report
...
**Acceptance Criteria:**
1. The readiness report is triggered manually from the migration review UI (Story 1.13) via "Run Validation", or automatically after `migration/generate-monolith-canvas` completes; it queues an Inngest function `migration/validate-pages` that processes each migrated page draft sequentially using `step.run`.
...
**Dependencies:** Story 1.13, CT Custom Objects API, CT project API (reference resolution), axe-core, Inngest 4.2.6
**FRs Covered:** FR39
**Complexity:** M
...
**Dependencies:** Story 1.2 (behavioral_events table), Story 1.8 (CT project connection)
**FRs Covered:** FR25
**Complexity:** XL
...
**Dependencies:** Story 5.1 (event ingestion pipeline), Story 1.8 (commercetools Customer Subscriptions), Prisma migration for `user_hash` column, `TENANT_STITCH_SECRET` per-tenant
**FRs Covered:** FR78
**Complexity:** L
...
**Dependencies (Phase 2):** Story 1.8 CT project connection, Story 5.1 + 5.2 (consent), Story 6.9 OAuth flow scaffold, Inngest 4.2.6, ad-platform OAuth credentials in `securedConfiguration`
**FRs Covered:** FR81
